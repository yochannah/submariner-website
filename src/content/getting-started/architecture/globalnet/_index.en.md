...
from the global CIDR and are used as egress IPs for all or selected pods in the namespace and take precedence over the cluster-level global IPs.

User can also configure `GlobalEgressIP`s for a specific set of pods in a namespace by using a `PodSelector` when creating `GlobalEgressIP`.
These `GlobalEgressIP`s will take precedence over any global IPs allocated at namespace or cluster level.
...
* Whenever `GlobalEgressIP`s are allocated, creates corresponding SNAT rules to convert source Ip from pod's Ip to the corresponding global IP.
* Whenever a service is allocated a global IP, creates an ingress rule to direct all traffic destined to the service's global IP to the
